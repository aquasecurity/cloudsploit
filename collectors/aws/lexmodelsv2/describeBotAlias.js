...
module.exports = function(AWSConfig, collection, retries, settings, scanAWSConfig, callback) {
    ...
    async.eachLimit(collection.lexmodelsv2.listBots[AWSConfig.region].data, 5, function(bot, cb){
        ...
        async.eachLimit(collection.lexmodelsv2.listBotAliases[AWSConfig.region][bot.botId].data.botAliasSummaries, 3, function(alias, pCb){
            ...
            helpers.makeCustomCollectorCall(lexmodelsv2, 'describeBotAlias', {botAliasId: alias.botAliasId,botId: bot.botId}, retries, null, null, null, settings, scanAWSConfig, AWSConfig, function(err, data) {
                if (err) {
