...
module.exports = function(AWSConfig, collection, retries, settings, scanAWSConfig, callback) {
    ...
    async.eachLimit(collection.managedblockchain.listNetworks[AWSConfig.region].data, 3, function(network, cb){
        ...
        async.eachLimit(collection.managedblockchain.listMembers[AWSConfig.region][network.Id].data.Members, 5, function(member, mcb){
            ...
            helpers.makeCustomCollectorCall(managedblockchain, 'getMember', {MemberId: member.Id,NetworkId: network.Id}, retries, null, null, null, settings, scanAWSConfig, AWSConfig, function(err, data) {
                if (err) {
