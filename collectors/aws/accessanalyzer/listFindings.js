...
module.exports = function(AWSConfig, collection, retries, settings, scanAWSConfig, callback) {
    ...
    async.eachLimit(collection.accessanalyzer.listAnalyzers[AWSConfig.region].data, 15, function(analyzer, cb) {
        ...
        function execute(nextToken) { // eslint-disable-line no-inner-declarations
            ...
            if (nextToken) {
                helpers.makeCustomCollectorCall(accessanalyzer, 'listFindings', localParams, retries, null, null, null, settings, scanAWSConfig, AWSConfig, paginateCb);
            } else {
                helpers.makeCustomCollectorCall(accessanalyzer, 'listFindings', params, retries, null, null, null, settings, scanAWSConfig, AWSConfig, paginateCb);
            }
        }
