...
module.exports = {
	...
	run: function(cache, settings, callback) {
		...
		for (p in listGpgKeys.data) {
			...
			if (key.created_at) {
				var returnMsg = 'User GPG key ' + keyName + ' was last rotated ' + helpers.functions.daysAgo(key.created_at) + ' days ago';
				var returnCode = 0;

				if (helpers.functions.daysAgo(key.created_at) > config.github_gpg_keys_rotated_fail) {
					returnCode = 2;
				} else if (helpers.functions.daysAgo(key.created_at) > config.github_gpg_keys_rotated_warn) {
					returnCode = 1;
				}
