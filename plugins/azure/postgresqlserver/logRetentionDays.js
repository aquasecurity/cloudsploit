    ...
    run: function (cache, settings, callback) {
        ...
        async.each(locations.servers.postgres, (location, rcb) => {
            ...
            configuration.forEach(config => {
                ...
                if (config.value > 3) {
                    helpers.addResult(results, 0, 'Log retention period is greater than 3 days', location, configId);
                } else {
                    helpers.addResult(results, 2, 'Log retention period is 3 days or less', location, configId);
                }
            });
