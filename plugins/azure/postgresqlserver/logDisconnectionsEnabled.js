    ...
    run: function (cache, settings, callback) {
        ...
        async.each(locations.servers.postgres, (location, rcb) => {
            ...
            configuration.forEach(config => {
                ...
                if (config.value === 'ON' ||
                    config.value === 'on') {
                    helpers.addResult(results, 0, 'Disconnection logs are enabled for the PostgreSQL Server configuration', location, configId);
                } else {
                    helpers.addResult(results, 2, 'Disconnection logs are disabled for the PostgreSQL Server configuration', location, configId);
                }
            });
