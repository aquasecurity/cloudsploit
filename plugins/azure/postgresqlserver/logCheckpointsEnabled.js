    ...
    run: function (cache, settings, callback) {
        ...
        async.each(locations.servers.postgres, (location, rcb) => {
            ...
            var configuration = configurations.data.filter(config => {
                return config.name === "log_checkpoints";
            });
            ...
            configuration.forEach(config => {
                ...
                if (config.value === 'ON' ||
                    config.value === 'on') {
                    helpers.addResult(results, 0, 'Log Checkpoints are enabled for the PostgreSQL Server configuration', location, configId);
                } else {
                    helpers.addResult(results, 2, 'Log Checkpoints are disabled for the PostgreSQL Server configuration', location, configId);
                }
            });
