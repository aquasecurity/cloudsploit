    ...
    describe('run', function() {
        ...
        it('should FAIL if postgresql server has Infrastructure Double Encryption disabled', function(done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('Infrastructure double encryption is not enabled for PostgreSQL Server Database');
                expect(results[0].region).to.equal('eastus');
                done()
            };
        ...
        it('should PASS if postgresql server has Infrastructure Double Encryption enabled', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Infrastructure double encryption is enabled for PostgreSQL Server Database');
                expect(results[0].region).to.equal('eastus');
                done()
            };
