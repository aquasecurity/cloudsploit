    ...
    describe('run', function() {
        ...
        it('should give passing result if Azure SQL Auditing is not using account storage', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Azure SQL Auditing not using account storage for the SQL server');
                expect(results[0].region).to.equal('eastus');
                done();
            };
        ...
        it('should give passing result if managed identity is configured for audit logs storage', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Managed identity is configured as authentication type for audit logs storage on the SQL server');
                expect(results[0].region).to.equal('eastus');
                done();
            };
        ...
        it('should give failing result if managed identity is not configured for audit logs storage', function(done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('Managed identity is not configured as authentication type for audit logs storage on the SQL server');
                expect(results[0].region).to.equal('eastus');
                done();
            };
