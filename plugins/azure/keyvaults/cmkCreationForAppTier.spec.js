...
describe('cmkCreationForAppTier', function() {
    describe('run', function() {
        ...
        it('should give passing result if CMK Creation for App Tier is enabled', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('CMK Creation for App Tier is enabled');
                expect(results[0].region).to.equal('eastus');
                done()
            };

            auth.run(createCache(null, [listVaults[1]], [getKeys[0]]), { app_tier_tag_sets: { hello: 'world' } }, callback);
        });

        it('should give failing result if CMK Creation for App Tier is disabled', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('CMK Creation for App Tier is disabled');
                expect(results[0].region).to.equal('eastus');
                done()
            };
