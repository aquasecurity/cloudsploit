    ...
    describe('run', function() {
        ...
        it('should give passing result if certificate has allowed key type', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Certificate key type is set to Allowed Key Types: RSA');
                expect(results[0].region).to.equal('eastus');
                done()
            };
        ...
        it('should give failing result if certificate does not have allowed key type', function(done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('Certificate key type is not set to Allowed Key Types: ECC');
                expect(results[0].region).to.equal('eastus');
                done()
            };
