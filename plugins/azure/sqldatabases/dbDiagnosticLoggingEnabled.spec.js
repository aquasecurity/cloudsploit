    ...
    describe('run', function() {
        ...
        it('should give passing result if diagnostic settings configured with minimum requirements', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Database diagnostic settings are configured with minimum requirements');
                expect(results[0].region).to.equal('eastus');
                done();
            };
        ...
        it('should give failing result if diagnostic settings not configured for the database', function(done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('Diagnostic settings are not configured for SQL database');
                expect(results[0].region).to.equal('eastus');
                done();
            };
