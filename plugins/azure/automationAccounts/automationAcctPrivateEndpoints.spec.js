...
const account = [
{
    ...
    "privateEndpointConnections" :[
        {
            "id": '/subscriptions/12345/resourceGroups/DefaultResourceGroup-EUS/providers/Microsoft.Automation/automationAccounts/Automate-12345-EUS2/privateEndpointConnections/a112345',
            "name": 'a112345',
    ...
    describe('run', function () {
        ...
        it('should give passing result if automation account has private endpoints configured', function (done) {
            ...
            automationAccountPrivateEndponits.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Private Endpoints are configured for Automation account');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
        ...
        it('should give failing result if automation account does not have private endpoints configured', function (done) {
            ...
            automationAccountPrivateEndponits.run(cache, {}, (err, results) => {
                ...
                expect(results[0].message).to.include('Private Endpoints are not configured for Automation account');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
