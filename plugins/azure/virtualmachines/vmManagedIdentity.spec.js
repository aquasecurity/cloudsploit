...
var vmManagedIdentity = require('./vmManagedIdentity');
...
describe('vmManagedIdentity', function() {
    describe('run', function() {
        it('should give passing result if no virtual machines', function(done) {
            const cache = createCache([]);
            vmManagedIdentity.run(cache, {}, (err, results) => {
        ...
        it('should give passing result if VM is using a managed identity', function(done) {
            const cache = createCache([virtualMachines[0]]);
            vmManagedIdentity.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('VM is using a managed identity to interact with other Azure Services');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });

        it('should give failing result if VM is not using a managed identity', function(done) {
            const cache = createCache([virtualMachines[1]]);
            vmManagedIdentity.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('VM is not using a managed identity to interact with other Azure Services');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
