    ...
    describe('run', function() {
        ...
        it('should give passing result if Trusted Launch is selected', function(done) {
            ...
            selectTrustedLaunch.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('trustedlaunch is selected as security type for virtual machine');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
        ...
        it('should give failing result if Trusted Launch is not selected', function(done) {
            ...
            selectTrustedLaunch.run(cache, {}, (err, results) => {
                ...
                expect(results[0].message).to.include('trustedlaunch is not selected as security type for virtual machine');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
