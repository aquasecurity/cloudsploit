    ...
    describe('run', function() {
        ...
        it('should give unknown result if unable to get media service', function(done) {
            ...
            amsContentKeyPolicy.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(3);
                expect(results[0].message).to.include('Unable to query for Content Key Policy');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
        ...
        it('should give passing result if content key policy exist', function(done) {
            ...
            amsContentKeyPolicy.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Azure Media Service has content key policy configured');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
        ...
        it('should give failing result if content key policy not exist', function(done) {
            ...
            amsContentKeyPolicy.run(cache, {}, (err, results) => {
                ...
                expect(results[0].message).to.include('Azure Media Service does not have content key policy configured');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
