...
var agAccessLogsEnabled = require('./agAccessLogsEnabled');
...
describe('agAccessLogsEnabled', function() {
    describe('run', function() {
        ...
        it('should give pass result if Application Gateway access logs are enabled', function(done) {
            const cache = createCache([appGateway[0]],[diagnosticSettings[0]]);
            agAccessLogsEnabled.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Application Gateway access logs are enabled');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });

        it('should give pass result if Application Gateway access logs are not enabled', function(done) {
            const cache = createCache([appGateway[0]],[diagnosticSettings[2]]);
            agAccessLogsEnabled.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Application Gateway access logs are not enabled');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
