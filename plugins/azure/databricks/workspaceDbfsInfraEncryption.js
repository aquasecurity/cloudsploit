...
module.exports = {
    ...
    severity: 'Medium',
    description: 'Ensures that DBFS root for Databricks premium workspace has infrastrucure encryption enabled.',
    ...
    recommended_action: 'Enable infrastructure level encryption for all Databricks premium workspace DBFS roots.',
        ...
        async.each(locations.databricks, function(location, rcb) {
            ...
            for (let workspace of databricks.data) {
                ...
                if (workspace.sku && workspace.sku.name && workspace.sku.name.toLowerCase()!='premium') {
                    ...
                    helpers.addResult(results, 0, 'DBFS root for databricks workspace has infrastructure level encryption enabled', location, workspace.id);
                }  else {
                    helpers.addResult(results, 2, 'DBFS root for databricks workspace does not have infrastructure level encryption enabled', location, workspace.id);
                }
            }
