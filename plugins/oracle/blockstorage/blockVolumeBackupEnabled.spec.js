    ...
    describe('run', function () {
        ...
        it('should give passing result if no volume records are found', function (done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('No Block Volumes present')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should give failing result if there is a block volume without a backup', function (done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('The following Block Volumes do not have a backup policy enabled')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should give passing result if all block volumes have backups', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(1)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('All Block Volumes have a backup policy enabled')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
