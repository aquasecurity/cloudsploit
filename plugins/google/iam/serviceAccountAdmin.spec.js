...
describe('serviceAccountAdmin', function () {
    describe('run', function () {
        ...
        it('should give failing result if a service account has admin privileges', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('The following service accounts have admin privileges');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if a service account has owner privileges', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('The following service accounts have owner privileges');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if a service account has editor privileges', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('The following service accounts have editor privileges');
                expect(results[0].region).to.equal('global');
                done()
            };
