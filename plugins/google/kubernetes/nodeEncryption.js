...
module.exports = {
    ...
    recommended_action: 'If you have confidential or sensitive data, it is recommended that you use Customer-Managed Keys to encrypt your GKE cluster nodes',
                    ...
                    clusters.data.forEach(cluster => {
                        ...
                        if (cluster.nodePools &&
                            ...
                            cluster.nodePools.forEach(nodePool => {
                                ...
                                if (nodePool.config && nodePool.config.bootDiskKmsKey && nodePool.config.bootDiskKmsKey.length) {
                                    let cryptoKey = keysObj[nodePool.config.bootDiskKmsKey];
                                    if (cryptoKey) {
                                ...
                                if (currentEncryptionLevel >= desiredEncryptionLevel) {
                                    helpers.addResult(results, 0,
                                        `Node Pool ${nodePool.name} is encrypted with ${currentEncryptionLevelStr} which is greater than or equal to ${desiredEncryptionLevelStr}`,
                                        region, resource);
                                } else {
                                    helpers.addResult(results, 2,
                                        `Node Pool ${nodePool.name} is encrypted with ${currentEncryptionLevelStr} which is less than ${desiredEncryptionLevelStr}`,
                                        region, resource);
                                }
                            });
                        } else {
