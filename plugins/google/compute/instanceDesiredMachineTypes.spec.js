    ...
    describe('run', function () {
        ...
        it('should FAIL if VM instance is not of the desired machine type', function (done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('Google Cloud Virtual Machine instance does not have the desired machine type');
                done()
            };
        ...
        it('should PASS if VM instance is of the desired machine type', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.equal('Google Cloud Virtual Machine instance has the desired machine type');
                done()
            };
