...
describe('timestreamDatabaseEncrypted', function () {
    describe('run', function () {
        it('should PASS if Timestream Databases is encrypted with desired encryption level', function (done) {
            const cache = createCache([listDatabases[1]], listKeys, describeKey[0]);
            timestreamDatabaseEncrypted.run(cache, { timestream_databases_encryption: 'awscmk' }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Timestream Databases is encrypted with awscmk');
                done();
            });
        });


        it('should FAIL if Timestream Databases is not encrypted with desired encyption level', function (done) {
            const cache = createCache([listDatabases[0]], listKeys, describeKey[1]);
            timestreamDatabaseEncrypted.run(cache, { timestream_databases_encryption:'awscmk' }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Timestream Databasesis encrypted with awskms');
                done();
            });
        });
