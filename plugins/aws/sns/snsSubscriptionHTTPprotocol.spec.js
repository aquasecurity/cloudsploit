...
const snsSubscriptionHTTPprotocol = require('./snsSubscriptionHTTPprotocol');
...
describe('snsSubscriptionHTTPprotocol', function () {
    describe('run', function () {
        it('should PASS if SNS subscription is configure to use http protocol', function (done) {
            ...
            snsSubscriptionHTTPprotocol.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('SNS subscription is using HTTP protocol');
        ...
        it('should FAIL if SNS subscription is not configure to use http protocol', function (done) {
            ...
            snsSubscriptionHTTPprotocol.run(cache, {}, (err, results) => {
                ...
                expect(results[0].message).to.include('SNS subscription is not using HTTP protocol');
                expect(results[0].region).to.equal('us-east-1');
                done();
            });
        });
