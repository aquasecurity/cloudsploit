...
module.exports = {
    ...
    more_info: 'You can configure Amazon Connect instance to save attachments to be saved on S3. When you save ' +
    ...
    apis: ['Connect:listInstances', 'Connect:instanceAttachmentsStorageConfigs', 'KMS:listKeys', 'KMS:describeKey'],
    settings: {
        connect_attachments_encryption_level: {
            name: 'Connect Attachments Encryption Level',
    ...
    run: function(cache, settings, callback) {
        ...
        async.each(regions.connect, function(region, rcb){
            ...
            for (let instance of listInstances.data) {
                ...
                var instanceAttachmentsStorageConfigs = helpers.addSource(cache, source,
                    ['connect', 'instanceAttachmentsStorageConfigs', region, instance.Id]);

                if (!instanceAttachmentsStorageConfigs || instanceAttachmentsStorageConfigs.err || !instanceAttachmentsStorageConfigs.data ||
                    !instanceAttachmentsStorageConfigs.data.StorageConfigs) {
                    helpers.addResult(results, 3,
                        `Unable to describe Connect instance Attachments storage config: ${helpers.addError(instanceAttachmentsStorageConfigs)}`,
                        region, resource);
                    continue;
                }

                if (!instanceAttachmentsStorageConfigs.data.StorageConfigs.length) {
                    helpers.addResult(results, 0,
                        'Connect instance does not have any storage config for Attachmentss',
                        region, resource);
                    continue;
                }

                let storageConfig = instanceAttachmentsStorageConfigs.data.StorageConfigs[0];
                ...
                if (storageConfig.S3Config) {
                    if (storageConfig.S3Config.EncryptionConfig &&
                        ...
                        if (currentEncryptionLevel >= desiredEncryptionLevel) {
                            helpers.addResult(results, 0,
                                `Connect instance is using ${currentEncryptionLevelString} for Attachmentss encryption\
                        ...
                        helpers.addResult(results, 2,
                            'Connect instance does not have encryption enabled for Attachmentss',
                            region, resource);
                    }
                } else {
                    helpers.addResult(results, 3,
