...
var es = require('./esNodeToNodeEncryption');
...
describe('esNodeToNodeEncryption', function () {
    describe('run', function () {
        ...
        it('should give error result if ES encryption config is disabled', function (done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('ES domain is not configured to use node-to-node encryption')
                done()
            };
        ...
        it('should give passing result if ES encryption config is enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('ES domain is configured to use node-to-node encryption')
                done()
            };
