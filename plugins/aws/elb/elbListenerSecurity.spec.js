...
const elbListenerSecurity = require('./elbListenerSecurity');
...
describe('elbListenerSecurity', function () {
    describe('run', function () {
        it('should PASS if ELB listener is using secure protocols only', function (done) {
            const cache = createCache([describeLoadBalancers[0]]);
            elbListenerSecurity.run(cache, {}, (err, results) => {
        ...
        it('should FAIL if ELB is using insecure protocols', function (done) {
        ...
        it('should FAIL if ELB is not using any listeners', function (done) {
            const cache = createCache([describeLoadBalancers[1]]);
            elbListenerSecurity.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
