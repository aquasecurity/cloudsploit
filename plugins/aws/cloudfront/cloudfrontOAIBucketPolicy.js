...
module.exports = {
    ...
    recommended_action: 'Review the access policy for S3 bucket which is an origin to a CloudFront distribution. Make sure S3 bucket is origin to only one distribution. ' +
        'Modify S3 bucket access policy to only allow CloudFront OAI for associated CloudFront distribution only and restrict access from any other source.',
    ...
    run: function(cache, settings, callback) {
        ...
        async.each(Object.entries(s3BucketAssociations), function([bucketName, cfDistributions], cb){
            ...
            if (getBucketPolicy && getBucketPolicy.err &&
                ...
                helpers.addResult(results, 2,
                    `No bucket policy found for bucket: ${bucketName}`,
                    'global', `arn:aws:s3:::${bucketName}`);
            }
            ...
            if (unknownPrincipals.length || restrictedOrigins.length) {
                if (unknownPrincipals.length) {
                    helpers.addResult(results, 2,
                        `S3 bucket is origin to "${distributionId}" and allows access to these unknown souces: ${unknownPrincipals.join(', ')}`,
                        'global', `arn:aws:s3:::${bucketName}`);
                }
                if (restrictedOrigins.length) {
                    helpers.addResult(results, 2,
                        `S3 bucket is origin to "${distributionId}" and does not allow access to these CloudFront OAIs: ${restrictedOrigins.join(', ')}`,
                        'global', `arn:aws:s3:::${bucketName}`);
                }
            } else {
                helpers.addResult(results, 0,
                    `S3 bucket is origin to only one distribution which is: ${distributionId}`, 'global', `arn:aws:s3:::${bucketName}`);
            }
