...
module.exports = {
    ...
    more_info: 'Function URLs without authentication (AuthType=NONE) are publicly invokable and may pose security risks.',
    link: 'https://docs.aws.amazon.com/lambda/latest/dg/lambda-urls.html',
    recommended_action: 'Set AuthType to AWS_IAM or restrict access via resource-based policy.',
        ...
        async.each(regions.lambda, function(region, rcb){
            ...
            async.each(listFunctions.data, function(lambdaFunc, fcb){
                ...
                var functionUrlConfig = helpers.addSource(cache, source,
                    ['lambda', 'getFunctionUrlConfig', region, lambdaFunc.FunctionName]);

                if (!functionUrlConfig || functionUrlConfig.err) {
                    helpers.addResult(results, 0,
                        'No Function URL configured', region, resource);
                    return fcb();
                }

                if (functionUrlConfig.data && functionUrlConfig.data.AuthType === 'NONE') {
                    helpers.addResult(results, 2,
                        'Function URL is public (AuthType=NONE)', region, resource, functionUrlConfig.data.FunctionUrl);
                } else if (functionUrlConfig.data && functionUrlConfig.data.AuthType) {
                    helpers.addResult(results, 0,
                        `Function URL exists but is secure (AuthType=${functionUrlConfig.data.AuthType})`, region, resource, functionUrlConfig.data.FunctionUrl);
                }
                fcb();
            });
            rcb();
