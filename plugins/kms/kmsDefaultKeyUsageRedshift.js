...
module.exports = {
	title: 'KMS Default Key Usage for redshift',
	category: 'KMS',
	description: 'Checks redshift service to ensure the default KMS key is not being used',
	...
	apis: ['Redshift:describeClusters', 'KMS:listKeys', 'KMS:describeKey'],
        ...
        async.each(helpers.regions.kms, function(region, rcb) {
			...
			if (!(describeClusters) || describeClusters.err || !describeClusters.data) {
				helpers.addResult(results, 3,
					'Unable to query for redshift: ' + helpers.addError(describeClusters), region);
					return rcb();
            }

            if(!(describeClusters.data.length)){
				helpers.addResult(results, 0, 'No redshift data Found', region);
				return rcb();
            }
