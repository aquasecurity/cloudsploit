...
module.exports = {
	title: 'KMS Default Key Usage for RDS',
	category: 'KMS',
	description: 'Checks RDS service to ensure the default KMS key is not being used',
	...
	apis: ['RDS:describeDBInstances', 'KMS:listKeys', 'KMS:describeKey'],
	...
	run: function(cache, settings, callback) {
        ...
        async.each(helpers.regions.kms, function(region, rcb) {
			...
			if (!(describeDBInstances) || describeDBInstances.err || !describeDBInstances.data) {
				helpers.addResult(results, 3,
					'Unable to query for RDS: ' + helpers.addError(describeDBInstances), region);
					return rcb();
            }

            if(!(describeDBInstances.data.length)){
				helpers.addResult(results, 0, 'No RDS data Found', region);
				return rcb();
            }
                ...
                for (i in defaultKeys){
                    for (j in services){
                        if (defaultKeys[i] === services.KMSKey){
							reg++;
                            helpers.addResult(results, 2, 'defult kms key in use: '+defaultKeys[i], region, defaultKeys[i]);
                        }
                    }
				}
