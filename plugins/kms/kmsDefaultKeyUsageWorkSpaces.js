...
module.exports = {
	title: 'KMS Default Key Usage for workspaces',
	category: 'KMS',
	description: 'Checks WorkSpaces service to ensure the default KMS key is not being used',
	...
	apis: ['workspaces:describeWorkspaces', 'KMS:listKeys', 'KMS:describeKey'],
	...
	run: function(cache, settings, callback) {
        ...
        async.each(helpers.regions.kms, function(region, rcb) {
            ...
            if (!describeWorkspaces) return rcb();
            ...
            if(!(describeWorkspaces.data.length)){
				helpers.addResult(results, 0, 'No workspaces data Found', region);
				return rcb();
            }
