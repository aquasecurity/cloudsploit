...
module.exports = {
	title: 'KMS Default Key Usage for EBS',
	category: 'KMS',
	description: 'Checks EC2/EBS service to ensure the default KMS key is not being used',
	...
	apis: ['EC2:describeVolumes', 'KMS:listKeys', 'KMS:describeKey'],
	...
	run: function(cache, settings, callback) {
        ...
        async.each(helpers.regions.kms, function(region, rcb) {
			...
			if (!(describeVolumes) || describeVolumes.err || !describeVolumes.data) {
            ...
            if(!(describeVolumes.data.length)){
				helpers.addResult(results, 0, 'No EBS data Found', region);
				return rcb();
            }
